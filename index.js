...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (
    ...
    if (action === "super_calisthenics") {
      ...
      // Отправляем сообщение с основным меню
      await ctx.reply(
        "Наш флагманский курс — Calisthenics light — это инновационный онлайн-курс, где тренировки адаптируются под ваш уровень физической подготовки и цели.\nВоспользуйтесь нижним меню, чтобы выбрать нужную команду.",
        {
          reply_markup: new Keyboard()
            .text("📝 Записаться на курс")
            .row()
  ...
  if (action === "deposit" || action === "deposit_ds_12") {
    ...
    try {
      ...
      if (userInfo) {
        ...
        if (keyboard) {
          await ctx.reply(
            "Здорово, что ты с нами!\nВыбери подходящий тариф из списка ниже — после оплаты просто запишись на тренировку через наш Telegram-чат или напрямую у своего тренера 💬💕\n\nЕсли возникнут вопросы, смело пиши Никите (@IDC_Manager) — он всегда на связи и с радостью поможет!",
            {
