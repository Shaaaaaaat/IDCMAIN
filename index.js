  ...
  ds: {
    RUB: [
      ...
      },
      {
        ...
        callback_data: "buy_11400_ds_rub",
      },
      {
        text: "36 занятий (25 200₽) — действует 18 недель",
...
bot.on("callback_query:data", async (ctx) => {
  ...
  if (action === "exercises_1") {
    const webhookUrl = "https://hook.eu1.make.com/7ef44ign27ukjw9lo2renkj4b3grfsnv";
    try {
      await axios.post(webhookUrl, { tg_id: ctx.from.id });
      await ctx.answerCallbackQuery({ text: "Отправлено" });
    } catch (error) {
  ...
  if (action === "yes_interesting") {
    const webhookUrl = "https://hook.eu1.make.com/tg8fh9rerez051exk1f2egexbfh3x9o9";
    try {
